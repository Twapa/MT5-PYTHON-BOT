...
class Technicals():
    ...
    def fetch_candles(self, row_count, candle_time):
        df = self.api.fetch_candles(self.pair, count=row_count)
    ...
    def process_candles(self, df):
        ...
        df[short_col] = df.close.rolling(window=self.settings.short_ma).mean()
        df[long_col] = df.close.rolling(window=self.settings.long_ma).mean()
        ...
        log_cols = ['time','tick_volume','close','spread','PAIR',short_col,long_col,short_prev,long_prev ,'D_PREV','D_NOW']
